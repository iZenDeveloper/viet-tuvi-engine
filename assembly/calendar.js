const PI = Math.PI;
export function abiVersion() { return 1; }
export function julianDay(day, month, year) {
    const a = (14 - month) / 12, y = year + 4800 - a, m = month + 12 * a - 3;
...
function sunSector(day, tz) { return Math.floor(sunLongitude(day - 0.5 - tz / 24) / PI * 6); }
function month11(year, tz) {
    const off = julianDay(31, 12, year) - 2415021, k = Math.floor(off / 29.530588853), nm = newMoonDay(k, tz);
    return sunSector(nm, tz) >= 9 ? newMoonDay(k - 1, tz) : nm;
}
function leapOffset(a11, tz) {
    const k = Math.floor((a11 - 2415021.076998695) / 29.530588853 + 0.5);
    let last = 0, i = 1, arc = sunSector(newMoonDay(k + i, tz), tz);
    do {
        last = arc;
        i++;
...
export function solarToLunarPacked(day, month, year, tz) {
    const dayNumber = julianDay(day, month, year), k = Math.floor((dayNumber - 2415021.076998695) / 29.530588853);
    let monthStart = newMoonDay(k + 1, tz);
    if (monthStart > dayNumber)
        monthStart = newMoonDay(k, tz);
    let a11 = month11(year, tz), b11 = a11, lunarYear;
    if (a11 >= monthStart) {
        lunarYear = year;
        a11 = month11(year - 1, tz);
    }
    else {
        lunarYear = year + 1;
        b11 = month11(year + 1, tz);
    }
    const lunarDay = dayNumber - monthStart + 1, diff = (monthStart - a11) / 29;
    const leapYear = b11 - a11 > 365;
    let lunarMonth = diff + 11, leap = false;
    if (leapYear) {
        const leapDiff = leapOffset(a11, tz);
        if (diff >= leapDiff) {
            lunarMonth = diff + 10;
            if (diff == leapDiff)
                leap = true;
    ...
    if (lunarMonth >= 11 && diff < 4)
        lunarYear--;
    return (lunarYear * 10000 + lunarMonth * 100 + lunarDay) * 2 + (leap ? 1 : 0);
}
