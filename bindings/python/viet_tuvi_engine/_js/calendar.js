...
const sunSector = (day, tz) => Math.floor(sunLongitude(day - 0.5 - tz / 24) / PI * 6);
...
export function solarToVietnameseLunar(day, month, year, timezoneHours = 7) {
    const dayNumber = jdFromDate(day, month, year), k = Math.floor((dayNumber - 2415021.076998695) / 29.530588853);
    let monthStart = newMoonDay(k + 1, timezoneHours);
    if (monthStart > dayNumber)
        monthStart = newMoonDay(k, timezoneHours);
    let a11 = month11(year, timezoneHours), b11 = a11, lunarYear;
    if (a11 >= monthStart) {
        lunarYear = year;
        a11 = month11(year - 1, timezoneHours);
    }
    else {
        lunarYear = year + 1;
        b11 = month11(year + 1, timezoneHours);
    }
    ...
    if (leapYear) {
        const leapDiff = leapOffset(a11, timezoneHours);
        if (diff >= leapDiff) {
            lunarMonth = diff + 10;
            if (diff === leapDiff)
                leap = true;
