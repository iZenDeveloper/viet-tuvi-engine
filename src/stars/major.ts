...
export const majorMetadata: Record<string, {
  element: string;
  yinYang: 'yin' | 'yang';
  group: 'tu-vi' | 'thien-phu';
}> = {
...
export function tuViBranch(lunarDay: number, cuc: number) {
  let extra = 0;
  while ((lunarDay + extra) % cuc !== 0) extra++;
  const quotient = (lunarDay + extra) / cuc;
  return ((2 + quotient + (extra % 2 === 0 ? extra : -extra) - 1) % 12 + 12) % 12;
}

export function majorStarBranches(tuVi: number) {
