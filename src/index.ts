...
export interface CalculateInput {
  localDateTime: string; timezoneOffsetMinutes: number; gender: Gender;
  trueSolarTime?: boolean; location?: { city?: string; longitude?: number };
  tradition?: Tradition; asOfYear?: number;
  asOfDate?: string;
  include?: { daiHan?: boolean; tieuHan?: boolean; luuNien?: boolean; luuNguyet?: boolean; luuNhat?: boolean; phiHoa?: boolean };
}
export interface Star { code: string; nameVi: string; kind: 'major'|'auxiliary'|'transformation'; palaceIndex: number; element?:string; yinYang?:'yin'|'yang'; group?:string; }
export interface Palace { code: string; nameVi: string; index: number; branch: string; isMenh: boolean; isThan: boolean; stars: string[]; }
export interface ChartFact { code:string; text:{vi:string;en:string}; evidence:string[] }
export interface ChartWarning { code:string; message:{vi:string;en:string}; severity:'info'|'warning' }
export interface PhiHoaFlight { code:string;sourcePalaceIndex:number;targetPalaceIndex:number;sourceStem:string;starCode:string;transformation:'loc'|'quyen'|'khoa'|'ky' }
export interface ProvenanceSource { url:string; role:'implemented-rule'|'reference-only'|'comparison-oracle' }
export interface TuViChart {
  input: CalculateInput; palaces: Palace[]; stars: Star[]; cuc: { code:string; nameVi:string; element:string; number:number };
  metadata: { engine:string; version:string; schemaVersion:string; ruleSetVersion:string; methodology:string; calculatedAt:string; capabilities:string[]; sources:ProvenanceSource[] };
  audit: { rule:string; value:string; source:string; version:string }[];
  timeline: { daiHan?: { startAge:number; endAge:number; palaceIndex:number }[]; tieuHan?: { year:number; nominalAge:number; palaceIndex:number }; luuNien?: { year:number; palaceIndex:number }[]; luuNguyet?:{asOfDate:string;lunarMonth:number;palaceIndex:number};luuNhat?:{asOfDate:string;lunarDay:number;palaceIndex:number} };
  relations: { code:string; type:'xung'|'tam-hop'|'luc-hop'|'chieu'; from:number; to:number }[];
  facts: ChartFact[];
...
const normalizeCity=(value:string)=>value.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/đ/g,'d').replace(/[^a-z0-9]/g,'');
export function listVietnamCities(){return vietnamCities.map(city=>({...city}));}
function findCity(value?:string){if(!value)return undefined;const key=normalizeCity(value);return vietnamCities.find(c=>[c.code,c.nameVi,c.nameEn].some(v=>normalizeCity(v)===key)||(['tphcm','hcmc'].includes(key)&&c.code==='vn-ho-chi-minh'));}
...
const majorMetadata:Record<string,{element:string;yinYang:'yin'|'yang';group:'tu-vi'|'thien-phu'}>={
...
function tuViBranch(lunarDay:number,cuc:number) {
  let extra=0; while((lunarDay+extra)%cuc!==0) extra++;
  const quotient=(lunarDay+extra)/cuc;
  return ((2+quotient+(extra%2===0?extra:-extra)-1)%12+12)%12;
}
function majorStarBranches(tuVi:number) {
...
export function createGroundedPrompt(chart: TuViChart, locale: 'vi'|'en' = 'vi') {
...
export function renderSvg(chart: TuViChart): string {
  ...
  const cells = chart.palaces.map(p => {
    ...
    for(const name of names){const candidate=line?`${line}, ${name}`:name;if(candidate.length>19&&line){lines.push(line);line=name;}else line=candidate;}
